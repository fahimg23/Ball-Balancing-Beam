...
int balancedPos = 69; //  OCR0B = 69, servo angle that balances the beam (straigtens it)
...
void loop(){
  ...
  if(distance1 > 0 && distance1 < 53){ //filters out fluctuating values which occur after 50 cm
    ...
    if((distance1 <= prevDist1 + cushion) && (distance1 >= prevDist1 - cushion)){
      ...
      //y = -25/46x + 65, y  = OCR0B, x = error. This is a function to map an error of 0 (ball is in the middle) to 65 and an error of 46 (ball is touching left ultrasonic sensor) to 40 (move beam all the way down)
      //errorMath1 = (float)((-0.54 * (float)(error1)) + 65.0);
      velocityMath1 = ((float)distance1 - (float)prevDist1)/dt;
      Serial.print("velocity1: ");
      Serial.println(velocityMath1);

      OCR0B = (int)(middlePos + error1 + velocityMath1*Kd1);
      Serial.println(OCR0B);
      //OCR0B = (int)(errorMath1);
      prevDist1 = distance1;
    }
  }
    ...
    if((distance2 <= prevDist2 + cushion) && (distance2 >= prevDist2 - cushion)){
      ...
      //y = 25/51x + 65, y  = OCR0B, x = error. This is a function to map an error of 0 (ball is in the middle) to 65 and an error of 51 (ball is touching left ultrasonic sensor) to 90 (move beam all the way up)
      //errorMath2 = (float)((0.49 * (float)(error2)) + 65.0);
      velocityMath2 = ((float)distance2 - (float)prevDist2)/dt;
      Serial.print("velocity2: ");
      Serial.println(velocityMath2);
      OCR0B = (int)(middlePos + error2 + velocityMath2*Kd2);
      //OCR0B = (int)(middlePos + velocityMath2*Kd2);
      // Kd2*velocityMath2
      prevDist2 = distance2;
    }
